...
ystar <-
X %*% b +
2/3 * (X[ , 1]+1/2) * (X[ , 2]-1/3) * (X[ , 3]+1/2) -
3/2 * (X[ , 3]+0) * (X[ , 4]+1/3) * (X[ , 5]-0) * (X[ , 6]-1/5) +
2/3 * (X[ , 6]+1/3) * (X[ , 7]-1/3) -
3/2 * (X[ , 1]-1/3) * (X[ , 2]+0) * (X[ , 4]-0) * (X[ , 5]-0) * (X[ , 7]-1/3)


yall <- ystar + rnorm(N, 0, 3)
...
which.min(xxers_mm)
cat("\n", "Best testing RMSE:", xxers_mm[ which.min(xxers_mm) ], " -- occurs at m =", which.min(xxers_mm), "\n")
plot( 0:(length(xxers_mm)-1), xxers_mm, type="l", col="#22FF11", lwd=3, main="Test RMSE ~vs~ First # Columns of What")




xwmags <- f_mact_find_polys(xxls_fit=xxls_out)
plot(xwmags, type="h")
...
bst <- xgb.train(param, dtrain, nrounds = 200)
cat( difftime(Sys.time(), xxnow, units="mins"), "\n" )
...
## model %>% fit(U_train, y_train, epochs = 200) ##
